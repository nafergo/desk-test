(function(global, factory) {
    ...
    function getCoveringElement(target) {
        ...
        return target.ownerDocument.elementsFromPoint(left + width / 2, top).find(el => !el.contains(target) && !hasClass(el, "uk-togglable-leave") && (hasPosition(el, "fixed") && zIndex(parents(target).reverse().find(parent2 => !parent2.contains(el) && !hasPosition(parent2, "static"))) < zIndex(el) || hasPosition(el, "sticky") && parent(el).contains(target)));
    }
    ...
    var sticky = {
        ...
        update: [ {
            read({
                ...
                sticky = maxScrollHeight && !this.showOnUp && start + offset$1 === topOffset && end === Math.min(maxScrollHeight, parseProp(true, this.$el, 0, true) - elHeight - offset$1 + overflow) && css(parent(this.$el), "overflowY") === "visible";
                return {
                    start: start,
                    end: end,
                    offset: offset$1,
